...
function socialApiBases(): string[] {
  ...
  if (
    h === "readyforrobots.com" ||
    h === "www.readyforrobots.com" ||
    h.endsWith(".readyforrobots.com")
  ) {
  ...
  for (let i = 0; i < bases.length; i++) {
    ...
    try {
      const res = await fetch(
        `${bases[i]}${path}`,
        liveFetchInit({ ...init, signal: controller.signal })
      );
      if (
        i < bases.length - 1 &&
      ...
      lastErr =
        e instanceof Error && e.name === "AbortError"
          ? new Error(
              "Request timed out — generation can take up to a minute. Try again."
            )
          : e instanceof Error
            ? e
...
function PostCard({
  ...
  return (
    <div
      className={`border bg-white shadow-sm ${isPosted ? "border-gray-200 opacity-70" : meta.border} rounded-xl overflow-hidden transition-opacity`}
    >
      <div
        className={`${isPosted ? "bg-gray-50" : meta.bg} border-b ${isPosted ? "border-gray-200" : meta.border} px-4 py-3 flex items-center justify-between gap-3 flex-wrap`}
      >
        <div className="flex items-center gap-3">
          ...
          <span
            className={`text-sm font-semibold ${isPosted ? "text-gray-500" : meta.text}`}
          >
            {isPosted ? "✓ Posted — " : ""}
            {meta.label}
          </span>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {post.source_industry && (
            <span className="text-[10px] border border-gray-200 bg-white text-gray-600 px-2 py-0.5 rounded font-mono">
              {post.source_industry}
            </span>
          )}
          {post.score != null && (
        ...
        {activeTab === "twitter" ? (
          <div className="space-y-3">
            ...
            <div className="flex items-center justify-between flex-wrap gap-2">
              <span
                className={`text-xs font-mono tabular-nums ${charColor(twitterText.length)}`}
              >
                {twitterText.length} chars
                {twitterText.length > TWITTER_SOFT_LIMIT && (
                  <span className="ml-1 text-red-600">⚠ may be truncated</span>
                )}
              </span>
...
export default function Social() {
  ...
  useEffect(() => {
    ...
    if (linkedinParam === "connected") {
      ...
      setLinkedinMsg(
        detail ? decodeURIComponent(detail) : "LinkedIn connect failed"
      );
  ...
  const studioAuthHeaders = useMemo((): Record<string, string> => {
    if (session?.access_token && isAdmin) {
      return Object.fromEntries(
        new Headers(authHeader(session.access_token)).entries()
      );
    }
  ...
  const studioAuthReady = Boolean(
    studioAuthHeaders.Authorization || studioAuthHeaders["X-Admin-Key"]
  );
  ...
  const connectLinkedIn = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API}/api/linkedin/connect-url?return_to=${encodeURIComponent(returnTo)}`,
        {
      ...
      if (res.status === 401) {
        ...
        throw new Error(
          typeof data.detail === "string"
            ? data.detail
            : "Invalid admin key — update the field below to match Fly ADMIN_KEY"
        );
      }
      if (!res.ok) {
        const detail =
  ...
  const publishToLinkedIn = async (text: string, articleUrl: string) => {
    ...
    try {
      ...
      if (resolveAdminKey()) persistAdminKey(resolveAdminKey());
      showLinkedinMessage(
        `Published to LinkedIn (${data.published_as || "ok"})`
      );
    } catch (e) {
      showLinkedinMessage(
  ...
  const applyData = (data: {
    ...
    if (data.cache_pending) {
      ...
      setError(
        data.message ||
          "Content is being prepared in the background. Retrying shortly."
      );
      return;
    }
    setPosts(data.posts || []);
  ...
  const fetchPosts = useCallback(async (opts?: { silent?: boolean }) => {
    ...
    const cachedEntry =
      readSurfaceCache<{
        ...
        date?: string;
        generated_at?: string;
      }>(SOCIAL_SESSION_KEY, SOCIAL_SESSION_TTL_MS) ??
      readSurfaceCache<{
        posts?: SocialPost[];
        date?: string;
        generated_at?: string;
      }>(SOCIAL_SESSION_KEY, SOCIAL_STALE_PAINT_MS);
    ...
    try {
      ...
      setCacheStatus(
        data.cache_status === "stale" ||
          res.headers.get("X-Social-Cache") === "stale"
          ? "stale"
          : null
      );
      applyData(data);
  ...
  useEffect(() => {
    ...
    if (li === "connected") {
      ...
      showLinkedinMessage(
        params.get("detail") || "LinkedIn connect failed",
        true
      );
      window.history.replaceState({}, "", "/social");
    }
    ...
    <div className="admin-workspace social-page min-h-screen">
      ...
      <PageHeroDark
        ...
        stats={[
          ...
          {
            label: "Marked",
            value: `${postedCount}/${totalLeadPosts || "—"}`,
            tone: "amber",
          },
      ...
      <main className="max-w-4xl mx-auto px-4 pb-8 pt-4">
        ...
        {cacheStatus === "stale" && (
          <p className="mb-3 text-xs text-amber-800 font-medium rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
            Showing cached posts while a fresh batch generates in the
            background.
          </p>
        )}
        {apiSlow && !error && (
          <p className="mb-3 text-xs text-amber-800 font-medium rounded-lg border border-amber-200 bg-amber-50 px-3 py-2">
            SIGNAL API is catching up — showing cached posts and retrying in the
            background.
          </p>
        )}
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              <p className="text-xs text-slate-300 mt-1 max-w-2xl">
                Step 1: authenticate Content Studio. Step 2: connect LinkedIn.
                Step 3: use{" "}
                <strong className="text-white">Publish to LinkedIn</strong> on
                any post below.
              </p>
            </div>
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              ...
              {session?.access_token && isAdmin ? (
                <p className="text-xs text-emerald-300">
                  Signed in as admin ({session.user.email}) — no admin key
                  needed unless you prefer one.
                </p>
              ) : session?.access_token ? (
                <p className="text-xs text-amber-200">
                  Signed in, but this account is not in ADMIN_EMAILS — paste
                  your Fly ADMIN_KEY below.
                </p>
              ) : (
                <p className="text-xs text-slate-400">
                  Not signed in — paste your Fly{" "}
                  <code className="text-slate-200">ADMIN_KEY</code> below, or{" "}
                  <Link
            ...
            <div className="rounded-xl border border-white/10 bg-white/5 p-4 space-y-3">
              ...
              {linkedinMsg && (
                <p
                  className={`text-xs font-mono ${linkedinMsgIsError ? "text-red-400" : "text-emerald-400"}`}
                >
                  {linkedinMsg}
                </p>
              )}
              <div className="flex flex-wrap gap-2 items-center">
                <a
                  href={
                    linkedinStatus?.organization_url ||
                    "https://www.linkedin.com/company/114404417/admin/dashboard/"
                  }
                  target="_blank"
                  rel="noopener noreferrer"
          ...
          <div className="mb-6 flex items-center justify-between gap-3 flex-wrap p-4 border border-gray-200 rounded-xl bg-white shadow-sm">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="text-xs px-3 py-1.5 rounded border border-emerald-200 text-emerald-700 hover:bg-emerald-50 disabled:opacity-40 bg-white"
              >
                ✓ Mark all as posted
              </button>
              <button
                ...
                className="text-xs px-4 py-1.5 rounded border border-violet-200 text-violet-700 hover:bg-violet-50 disabled:opacity-50 font-semibold bg-white"
              >
        ...
        {refreshing && (
          <div className="mb-4 p-3 border border-violet-200 rounded-xl bg-violet-50 text-xs text-violet-800 font-mono">
            Generating a fresh batch — usually takes 30–60 seconds. Your current
            posts stay visible below.
          </div>
        )}
        ...
        {loading && !posts?.length && (
          <div className="space-y-4">
            {[1, 2, 3, 4, 5].map(i => (
              <div
                key={i}
                className="border border-gray-200 rounded-xl h-48 animate-pulse bg-gray-100"
              />
            ))}
          </div>
        )}
        ...
        {error && !loading && !refreshing && !posts?.length && (
          <div className="border border-red-200 bg-red-50 rounded-xl p-6 text-center">
            ...
            <button
              ...
              className="text-xs border border-gray-300 px-3 py-1.5 rounded bg-white text-gray-700 hover:bg-gray-50"
            >
              Try again
            </button>
          </div>
        ...
        {!loading && !error && posts && posts.length > 0 && (
          <div
            className={`space-y-6 ${refreshing ? "opacity-60 pointer-events-none" : ""}`}
          >
            {posts.map((post, i) => (
              <PostCard
                ...
                isPosted={
                  post.company_id != null && postedIds.has(post.company_id)
                }
            ...
            <div className="border border-gray-200 bg-white rounded-xl p-4 text-center text-xs text-gray-600 space-y-2 shadow-sm">
              <p>
                Posts refresh every 4 hours. Mark as posted to rotate companies
                for 7 days.
              </p>
