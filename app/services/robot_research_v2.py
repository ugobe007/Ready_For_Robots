...
_CLAIM_KEYWORDS: dict[str, tuple[str, ...]] = {
    ...
    "end_effector": ("gripper", "hand", "end effector", "suction", "vacuum", "finger", "claw", "grasp", "arm", "arms", "manipulat", "dexter"),
...
def _claim_supported(predicate: str, value: Any, evidence: str) -> bool:
    ...
    if predicate in ("arm_count", "carrying_capacity", "reach_or_workspace", "battery_runtime"):
        ...
        cands.add(str(num))
        # Check if the digit appears as a standalone token OR within a compound form
        # (e.g., "7x2", "55lb"). This allows compounds while still blocking substring
        # false positives like "5" matching inside "155" when both are standalone tokens.
        found = False
        for c in cands:
            if not c:
                continue
            # Standalone token (space-bounded or at start/end)
            if f" {c} " in f" {ev} " or ev.startswith(f"{c} ") or ev.endswith(f" {c}"):
                found = True
                break
            # Compound form: digit followed by non-digit non-space (x, lb, kg, etc.)
            # or preceded by non-digit non-space
            import re
            if re.search(rf'(?:^|[^0-9]){re.escape(c)}(?:[^0-9 ]|$)', ev):
                found = True
                break
        if not found:
            return False
