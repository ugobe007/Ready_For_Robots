...
# ── Proxy to Next.js SSR server (runs on port 3000) ────────────────────────
# Frontend pages are served by Next.js with server-side rendering for instant page loads
import httpx

@app.get("/{full_path:path}", include_in_schema=False)
async def proxy_to_nextjs(full_path: str, request: Request):
    """Proxy frontend requests to Next.js server running on port 3000"""
    # Skip API routes - they're already handled above
    if full_path.startswith("api/"):
        return {"error": "API route not found"}, 404
    
    # Proxy to Next.js server
    nextjs_url = f"http://localhost:3000/{full_path}"
    ...
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(nextjs_url, timeout=10.0)
            return Response(
        ...
        # Fallback if Next.js server not running
        return {"error": "Frontend server unavailable", "details": str(e)}, 503