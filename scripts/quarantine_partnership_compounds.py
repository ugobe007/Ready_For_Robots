#!/usr/bin/env python3
"""
Quarantine companies whose names are partnership headline compounds (X and Y).

These rows are not buyer sales opportunities — one side is usually a vendor or
co-marketing partner. Soft-hide via ``is_internal=False`` (rectifier quarantine).

Dry-run (default):
  python3 scripts/quarantine_partnership_compounds.py

Apply:
  python3 scripts/quarantine_partnership_compounds.py --apply
"""
...
from app.services.lead_filter import is_partnership_compound_name
from app.services.pipeline_delete_policy import is_quarantined
from app.services.rectifier import quarantine


def main() -> None:
    parser = argparse.ArgumentParser(description="Quarantine X-and-Y partnership compound leads")
    ...
    try:
        ...
        for company in q.all():
            ...
            if not name:
                continue
            junk, reason = is_partnership_compound_name(name)
            if junk:
    ...
    report_path = args.report or str(
        _root / "reports" / f"partnership_compound_quarantine_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv"
    )
    ...
    print(f"Partnership compound candidates: {len(rows)}")
