...
export default function BillingSuccess() {
  ...
  useEffect(() => {
    ...
    if (!sessionId) {
      setStatus("error");
      setMessage(
        "Missing checkout session. If you were charged, contact support."
      );
      return;
    }
    ...
    void (async () => {
      try {
        ...
        setStatus("done");
        setMessage(
          "You're on Pro — full pipeline and SIGNAL research are unlocked."
        );
        window.setTimeout(() => setLocation("/pipeline"), 2500);
      } catch (err) {
        setStatus("error");
        setMessage(
          err instanceof Error ? err.message : "Could not confirm subscription"
        );
      }
    })();
