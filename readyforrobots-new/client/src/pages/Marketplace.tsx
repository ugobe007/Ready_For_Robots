...
const inputClass =
  "w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none placeholder:text-gray-400 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/30";
const labelClass =
  "mb-1 block text-[10px] font-bold uppercase tracking-widest text-gray-600";
...
const marketplaceTeaserFeatures = [
  {
    ...
    preview: [
      {
        label: "Buy-side",
        value: "Regional hospital system · warehouse AMR rollout",
      },
  ...
  },
  {
    ...
    preview: [
      ...
      {
        label: "Requirements",
        value: "12 technical · 4 compliance · ROI model",
      },
    ],
  },
  {
    ...
    preview: [
      {
        label: "Assets",
        value: "product_spec · case_study · pricing · compliance",
      },
      {
        label: "Visibility",
        value: "Private to buyer team until proposal stage",
      },
    ],
  },
  {
  ...
  },
  {
    ...
    preview: [
      ...
      {
        label: "Guardrail",
        value: "Secret references only — no raw API keys stored",
      },
    ],
  },
];
...
export default function Marketplace() {
  ...
  const loadWorkspace = useCallback(async () => {
    ...
    setMessage("");
    try {
      const [orgData, assetData, rfqData, docData, connectionData] =
        await Promise.all([
      ...
      setOrgForm(current => ({
        ...current,
        organization_type:
          orgData.profile?.organizationType || current.organization_type,
      ...
      setMessage(
        e instanceof Error ? e.message : "Could not load marketplace workspace"
      );
    } finally {
      setBusy(false);
    }
    ...
    try {
      await authFetch("/api/marketplace/rfqs", {
        ...
        body: JSON.stringify({
          ...
          status: "draft",
          due_at: rfqForm.due_at
            ? new Date(rfqForm.due_at).toISOString()
            : null,
          decision_makers: tryJson(rfqForm.decision_makers, []),
          technical_specs: tryJson(rfqForm.technical_specs, {}),
          workflow_process: {
            process: "buyer-defined",
            notes: rfqForm.timeline_summary,
          },
          ...
          requirements: splitLines(rfqForm.requirements).map(body => ({
  ...
  const createScheduleEvent = async (rfq: RfqRow) => {
    ...
    try {
      ...
      toast.error(
        e instanceof Error ? e.message : "Could not schedule RFP event"
      );
    }
  };
    ...
    try {
      await authFetch("/api/marketplace/commercial-documents", {
        ...
        body: JSON.stringify({
          ...
          due_at: docForm.due_at
            ? new Date(docForm.due_at).toISOString()
            : null,
    ...
    try {
      await authFetch("/api/marketplace/connections", {
        ...
        body: JSON.stringify({
          ...
          config: {
            guardrail: "No raw credentials stored in Ready For Robots",
          },
        }),
      });
  ...
  const issuePartnerApiKey = async (
    connectionId: string,
    ...
    try {
      const out = await authFetch(
        `/api/marketplace/connections/${connectionId}/api-keys`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
      ...
      toast.success(
        "Partner API key created — copy it now; it won't be shown again."
      );
      if (out?.apiKey) {
  ...
  return (
    <div className="marketplace-page min-h-screen flex flex-col">
      ...
      <main className="admin-workspace mx-auto w-full max-w-6xl flex-1 px-4 pb-12 pt-6">
        <AdminNav />
        {message && (
          <p className="mt-2 rounded-lg border border-red-300 bg-red-50 p-3 text-sm font-medium text-red-900">
            {message}
          </p>
        )}
        {busy && (
          <p className="mt-4 text-xs font-medium text-gray-600">Working...</p>
        )}
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <section className={cardClass}>
            <SectionTitle
              ...
              kicker={org?.team?.name || "Workspace"}
            />
            <div className="grid gap-3">
              <select
                ...
                onChange={e =>
                  setOrgForm(s => ({
                    ...s,
                    organization_type: e.target.value as "buyer" | "vendor",
                  }))
                }
              ...
              <Input
                ...
                onChange={display_name =>
                  setOrgForm(s => ({ ...s, display_name }))
                }
              />
              <Input
                label="Website"
                value={orgForm.website}
                onChange={website => setOrgForm(s => ({ ...s, website }))}
              />
              <Textarea
                label="Description"
                ...
                onChange={description =>
                  setOrgForm(s => ({ ...s, description }))
                }
              />
              <Textarea
                label="Decision makers JSON"
                value={orgForm.decision_makers}
                rows={3}
                onChange={decision_makers =>
                  setOrgForm(s => ({ ...s, decision_makers }))
                }
              />
              <button
          ...
          <section className={cardClass}>
            <SectionTitle
              ...
              kicker={`${assets.length} assets`}
            />
            <div className="grid gap-3">
              <select
                value={assetForm.asset_type}
                onChange={e =>
                  setAssetForm(s => ({ ...s, asset_type: e.target.value }))
                }
                className={inputClass}
              >
                {[
              ...
              <button
                ...
                className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-bold text-gray-700 shadow-sm"
              >
                Upload asset
              </button>
              <ListEmpty
                ...
                render={asset => `${asset.assetType}: ${asset.filename}`}
              />
            </div>
          </section>
          ...
          <section className={cardClass}>
            <SectionTitle
              title="Buyer RFP Builder"
              kicker={`${rfqs.length} RFPs`}
            />
            <div className="grid gap-3">
              <Input
                label="RFP title"
                value={rfqForm.title}
                onChange={title => setRfqForm(s => ({ ...s, title }))}
              />
              <Textarea
                label="Project description"
                value={rfqForm.project_description}
                rows={4}
                onChange={project_description =>
                  setRfqForm(s => ({ ...s, project_description }))
                }
              />
              <Textarea
              ...
              <Input
                ...
                onChange={due_at => setRfqForm(s => ({ ...s, due_at }))}
              />
              <Textarea
                label="Technical specs JSON"
                value={rfqForm.technical_specs}
                rows={3}
                onChange={technical_specs =>
                  setRfqForm(s => ({ ...s, technical_specs }))
                }
              />
              <button
              ...
              <ListEmpty
                ...
                action={rfq => (
                  <button
                    onClick={() => void createScheduleEvent(rfq)}
          ...
          <section className={cardClass}>
            <SectionTitle
              title="Quotes, Invoices, POs"
              kicker={`${documents.length} documents`}
            />
            <div className="grid gap-3">
              <select
                value={docForm.document_type}
                onChange={e =>
                  setDocForm(s => ({ ...s, document_type: e.target.value }))
                }
                className={inputClass}
              >
                {["proposal", "quote", "invoice", "purchase_order"].map(
                  value => (
                    <option key={value} value={value}>
                      {value}
                    </option>
              ...
              <Input
                ...
                value={docForm.title}
                onChange={title => setDocForm(s => ({ ...s, title }))}
              />
              <Input
                label="Document number"
                value={docForm.document_number}
                onChange={document_number =>
                  setDocForm(s => ({ ...s, document_number }))
                }
              />
              <Input
                label="Amount"
                value={docForm.amount}
                onChange={amount => setDocForm(s => ({ ...s, amount }))}
              />
              <button
                onClick={() => void createCommercialDocument()}
                className="rounded-lg border border-gray-200 bg-white px-3 py-2 text-xs font-bold text-gray-700 shadow-sm"
              >
                Create document
              </button>
              <ListEmpty
                items={documents}
                empty="No commercial documents yet."
                render={doc =>
                  `${doc.documentType}: ${doc.title || doc.documentNumber || doc.status}`
                }
              />
            </div>
          </section>
          ...
          <section className={`${cardClass} lg:col-span-2`}>
            <SectionTitle
              title="MCP / API Connections"
              kicker={`${connections.length} connections`}
            />
            <div className="grid gap-3 md:grid-cols-2">
              <Input
                label="Connection name"
                value={connectionForm.name}
                onChange={name => setConnectionForm(s => ({ ...s, name }))}
              />
              <Input
                label="MCP server URL"
                value={connectionForm.mcp_server_url}
                onChange={mcp_server_url =>
                  setConnectionForm(s => ({ ...s, mcp_server_url }))
                }
              />
              <Input
                label="API base URL"
                ...
                onChange={base_url =>
                  setConnectionForm(s => ({ ...s, base_url }))
                }
              />
              <Input
                ...
                onChange={secret_ref =>
                  setConnectionForm(s => ({ ...s, secret_ref }))
                }
              />
              <Textarea
                label="Allowed scopes, one per line"
                value={connectionForm.allowed_scopes}
                rows={4}
                onChange={allowed_scopes =>
                  setConnectionForm(s => ({ ...s, allowed_scopes }))
                }
              />
              <div className="rounded-xl border border-gray-200 bg-gray-50 p-3 text-xs text-gray-600">
                Store credentials in your secret manager, then paste only a
                reference here. Ready For Robots stores `secret_ref`, connection
                URLs, scopes, and config, not raw API keys.
              </div>
            </div>
            <button
              onClick={() => void createConnection()}
              className="mt-3 rounded-lg border border-amber-400/35 bg-amber-50 px-3 py-2 text-xs font-bold text-amber-900"
            >
            ...
            <div className="mt-3 space-y-2">
              <ListEmpty
                ...
                render={connection =>
                  `${connection.connectionType}: ${connection.name} (${connection.status})`
                }
              />
              {connections.map(connection => (
                <button
                  ...
                  type="button"
                  onClick={() =>
                    void issuePartnerApiKey(connection.id, connection.name)
                  }
  ...
  return (
    <div className="marketplace-page min-h-screen flex flex-col">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-4 pb-16 pt-6">
              ...
              <section key={feature.title} className={cardClass}>
                ...
                <div className="grid gap-2">
                  {feature.preview.map(row => (
                    <div
                      key={row.label}
                      className="rounded-xl border border-gray-200 bg-slate-50 px-3 py-2.5"
                    >
        ...
        <section
          className={`${cardClass} mt-4 border-amber-300 bg-gradient-to-r from-amber-50 to-white`}
        >
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-sm font-bold text-gray-900">
                Ready to publish an RFP or respond as a vendor?
              </p>
              <p className="mt-1 text-xs text-gray-500">
                Create a free account to save profiles, upload materials, and
...
function SectionTitle({ title, kicker }: { title: string; kicker: string }) {
  return (
    <div className="mb-4 flex items-center justify-between gap-3">
      ...
      <span className="rounded-full border border-gray-300 bg-slate-50 px-2 py-1 text-[10px] font-bold text-gray-600">
        {kicker}
      </span>
    </div>
  );
...
function ListEmpty<T>({
  ...
  if (!items.length)
    return (
      <p className="rounded-xl border border-gray-200 bg-slate-50 p-3 text-xs font-medium text-gray-600">
        {empty}
      </p>
    );
  return (
    <div className="grid gap-2">
      {items.slice(0, 5).map((item, index) => (
        <div
          key={index}
          className="flex items-center justify-between gap-3 rounded-xl border border-gray-200 bg-white p-3 text-xs text-gray-800"
        >
