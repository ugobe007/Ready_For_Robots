...
GitHub **Deploy frontend to Vercel** must have repo secrets `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`. Without them the job used to skip-green in ~7s and production did not move. It now **fails** until those secrets exist. `VERCEL_TOKEN` must not contain a space (CLI 59+); the workflow trims trailing paste whitespace. Vercel Git still builds PR **Preview** URLs; Production needs `vercel deploy --prebuilt --prod` (this workflow) or a promoted Preview.
