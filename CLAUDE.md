...
0a1. **Hermes retired:** do not spawn Hermes for FIND/Jobs. Do not follow `docs/hermes_intelligence_bridge.md` as production. Matcher + harness stay. Cursor pstack is the Jobs **release authority** (How / Act / Critic in `pstack/` + `scripts/pstack_release.py`). Not a customer chatbot. Not protocol chrome on `/` or Jobs CRM. No Jobs product PR without pstack checks. See `docs/hermes_retired.md` and `docs/pstack_jobs.md`.
