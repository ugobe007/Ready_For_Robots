...
- [`mixed_oem_sku_catalog.v1.json`](mixed_oem_sku_catalog.v1.json) — OEMs that ship mixed morphology (humanoid + serving + cleaning + quadruped) plus cleaning / mixed F&B lineups. Evidence-backed named SKUs only. Lucidbots is `cleaning_drone` (not a floor scrubber). Kaercher is robotic KIRA only. Live page extract still wins. Merged on ingest like the vertical overlay.
