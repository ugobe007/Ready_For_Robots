...
import { authEmailRejectReason, normalizeAuthEmail, otpNoAccountMessage } from "./authEmail";

describe("authEmail", () => {
  it("accepts ordinary work emails including plus tags", () => {
    expect(normalizeAuthEmail("Ops@AcmeRobotics.COM")).toBe("ops@acmerobotics.com");
  ...
  it("rejects empty, malformed, and reserved test domains", () => {
    ...
    expect(authEmailRejectReason("user@example.com")).toMatch(/real work email/i);
    expect(authEmailRejectReason("dev@foo.test")).toMatch(/real company email/i);
  });
  ...
  it("maps login OTP misses to a signup prompt instead of a raw GoTrue error", () => {
    expect(otpNoAccountMessage("Signups not allowed for otp")).toMatch(/signup page/i);
