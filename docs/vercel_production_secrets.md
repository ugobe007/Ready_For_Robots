...
The GitHub job **builds Vite in Actions**, packs `.vercel/output`, and runs `vercel deploy --prebuilt --prod`. That Vite step must receive `VITE_PUBLIC_SUPABASE_URL` and `VITE_PUBLIC_SUPABASE_ANON_KEY` (same public values as `fly.toml` `[build.args]`). Without them the bundle has no `supabase.co` host, `/signup` Google and GitHub buttons stay disabled, and magic-link does nothing. `vite.config.ts` fails the production build if those vars are missing. `scripts/vercel_production_smoke.py` **gates only on `https://readyforrobots.com`**: JS path ≠ `index-bxLpnQiT.js`, downloaded JS >100kB, canaries `Jobs for` and `lmoyydlhlgdyqbxkmkuz.supabase.co`. A `*.vercel.app` 404 (Deployment Protection / same-second race) is logged and ignored — `#105` aborted on `curl -f` of that host under `set -e` while the custom domain was already the new bundle.
