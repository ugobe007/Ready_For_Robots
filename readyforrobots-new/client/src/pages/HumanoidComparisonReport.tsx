    ...
    <div className="min-w-[72px]">
      <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
        <div
          ...
          style={{ width: `${pct}%`, background: RR.teal }}
        />
      </div>
      <span
...
export default function HumanoidComparisonReport() {
  ...
  const heroStats = report
    ? [
        {
          label: "Fleet tracked",
          value: fmtNum(metrics.fleet_total_robots),
          tone: "emerald" as const,
        },
        {
          label: "PoC or better",
          value: fmtPct(metrics.fleet_poc_or_better_pct),
          tone: "emerald" as const,
        },
        {
          label: "Top slice",
          value: fmtNum(metrics.robots_in_top_slice),
          tone: "white" as const,
        },
        {
          label: "Deploy signals",
          value: fmtNum(metrics.fleet_deployment_signal_count),
          tone: "amber" as const,
        },
      ]
    : undefined;
  ...
  return (
    <div className="robots-page min-h-screen">
      <Header />
      <PageHeroDark
        ...
        title={
          <>
            Humanoid <span className="text-emerald-400">Comparison Report</span>
          </>
        }
        ...
        actions={
          <div className="flex flex-wrap items-center gap-3">
            <ReportBtnDownload
              href={humanoidReportPdfUrl(TOP_N)}
              label="Download PDF"
            />
            <button
              type="button"
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        ...
        {!loading && error && (
          <ReportPanel accent="none">
            ...
            <Link
              href="/robots"
              className="mt-3 inline-flex text-[13px] font-semibold text-emerald-700 hover:underline"
            >
        ...
        {!loading && !error && report && (
          <div className="space-y-10">
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <ReportMetric
                label="Humanoids indexed"
                value={fmtNum(metrics.fleet_total_robots)}
              />
              <ReportMetric
                label="PoC / pilot / commercial"
                value={fmtPct(metrics.fleet_poc_or_better_pct)}
              />
              <ReportMetric
                label="Named customers (top slice)"
                value={fmtNum(metrics.robots_with_named_customers_top_slice)}
              />
              <ReportMetric
                label="News deployment headlines"
                value={fmtNum(metrics.news_deployment_headlines_top_slice)}
              />
            </div>
            ...
            <section>
              ...
              <ReportTitle id="comparison-table">
                Top {TOP_N} humanoids compared
              </ReportTitle>
              <p className="mt-1 text-[13px]" style={{ color: RR.textDim }}>
                HEIF dimensions (0–4) and composite index (0–100). Index ≈ HEIF
                × 25.
              </p>
              <div className="mt-4">
                <ReportTable
              ...
              <section>
                ...
                <ReportTable
                  ...
                  headers={[
                    ...
                    "Robot",
                    "Total",
                    ...(report.comparisons.peer_heif_matrix.dimension_labels ??
                      DIM_KEYS.map(k => DIM_LABELS[k])),
                  ]}
              ...
              report.comparisons.vendor_leaderboard.length > 0 && (
                <section>
                  ...
                  <ReportTable
                    ...
                    rows={report.comparisons.vendor_leaderboard
                      .slice(0, 12)
                      .map(row => [
                        <span
                          className="font-semibold"
            ...
            {report.comparisons?.index_vs_deployment &&
              report.comparisons.index_vs_deployment.length > 0 && (
                <section>
                  <ReportSectionLabel>
                    Capability vs deployment reality
                  </ReportSectionLabel>
                  <ReportTable
                    minWidth="520px"
                    headers={[
            ...
            {report.comparisons?.ranking_divergence &&
              report.comparisons.ranking_divergence.length > 0 && (
                <section>
                  <ReportSectionLabel>
                    Index vs deployment-weighted rank
                  </ReportSectionLabel>
                        ...
                        <ReportPanel key={row.name} accent="teal">
                          ...
                          >
                            ...
                            <span
                              ...
                              style={{ color: RR.textDim }}
                            >
                              index #{row.index_rank} · deploy #
                              {row.deployment_weighted_rank}
                              {row.rank_delta !== 0
                                ? ` · Δ${row.rank_delta > 0 ? "+" : ""}${row.rank_delta}`
                                : ""}
                            </span>
                          </p>
                          <ReportBodyText className="mt-1">
                            {row.commentary}
                          </ReportBodyText>
                        </ReportPanel>
                      ))}
            ...
            >
              ...
              <ReportBodyText className="mt-3">
                HEIF (Humanoid Engineering Intelligence Framework) scores six
                dimensions from HEIR 2026 research benchmarks and published
                specs. Deployment tiers combine catalog status, commercial
                deployment counts, and English/Chinese news headlines. Customer
                names are extracted from headlines — verify before citing.
              </ReportBodyText>
