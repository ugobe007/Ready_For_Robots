...
const VOICE_FEEDBACK = [
  {
    label: "Too generic",
    instruction: "Use one specific signal hook and remove generic claims.",
  },
  {
    label: "Too long",
    instruction:
      "Keep the email under 140 words unless the customer has already replied.",
  },
  {
    label: "Too salesy",
    instruction:
      "Use lower-pressure language and admit uncertainty when the fit is not confirmed.",
  },
  {
    label: "Better hook needed",
    instruction:
      "Lead with the clearest why-now signal before explaining Ready For Robots.",
  },
  {
    label: "More edge",
    instruction:
      "Make the language sharper and more commercially confident without adding hype.",
  },
  {
    label: "Founder-level",
    instruction:
      "Explain the pipeline logic like an operator speaking to a technical founder.",
  },
  {
    label: "Add off-ramp",
    instruction:
      "Include a graceful exit if the signal is not strong enough or the timing is wrong.",
  },
  {
    label: "Good tone",
    instruction: "Keep this tone: concise, human, specific, and low-pressure.",
  },
];
...
export default function Crm() {
  ...
  const crmReturnHref = (() => {
    ...
    if (!isJobsHandoffSrc(params.get("src")))
    ...
    async (optedIn: boolean) => {
      ...
      try {
        ...
        if (optedIn && snap?.url) {
          ...
          payload.seed_jobs = (snap.jobs || [])
            .slice(0, CRM_UNLOCKED_JOBS)
            .map(job => ({
              job_key: job.job_key,
              title: job.title,
  ...
  useEffect(() => {
    ...
    setSuggestions([]);
    setStyleApproved(
      selectedAccount.outreach_stage === "draft_approved" ||
        selectedAccount.outreach_stage === "intro_sent"
    );
  ...
  const sendBlockers = (): string[] => {
    ...
    if (!styleApproved)
      blockers.push('click "Approve draft" to confirm the message');
    return blockers;
  };
  ...
  const scoutStyleGuidance = () => {
    const pieces = [
      ...
      settings?.scout_preferred_channel &&
      settings.scout_preferred_channel !== "email"
        ? `Preferred next step: suggest a ${settings.scout_preferred_channel}.`
        : "",
      settings?.scout_meeting_preference
        ? `Scheduling preference: ${settings.scout_meeting_preference}`
        : "",
    ].filter(Boolean);
  ...
  const applyVoiceFeedback = (instruction: string) => {
    ...
    setStyleApproved(false);
    setMsg(
      "Style feedback saved locally. Click Draft outreach to apply it to the next draft."
    );
  };
  ...
  const draftWithScout = async () => {
    ...
    try {
      const result = await authFetch(
        `/api/crm/accounts/${selectedAccount.id}/draft-outreach`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const saveDraft = async () => {
    ...
    try {
      const saved = (await authFetch(
        `/api/crm/accounts/${selectedAccount.id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      setAccounts(prev =>
        prev.map(a => (a.id === selectedAccount.id ? { ...a, ...saved } : a))
      );
  ...
  const generatePlan = async () => {
    ...
    try {
      const result = (await authFetch(
        `/api/crm/accounts/${selectedAccount.id}/generate-plan`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const sendWithScout = async () => {
    ...
    try {
      const result = await authFetch(
        `/api/crm/accounts/${selectedAccount.id}/send-outreach`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
                    ...
                    <table className="w-full text-base">
                      ...
                      <tbody>
                        {accounts.map(a => (
                          ...
                          >
                            <td className="px-3 py-2.5 font-display text-lg font-bold text-white">
                              {a.name}
                            </td>
                  ...
                  <div className="flex">
                    ...
                    <div className="sb-surface-body flex-1">
                      <div className="mb-2 flex items-start justify-between gap-2 border-b border-slate-700 pb-2">
                        ...
                        <span
                          className="inline-flex px-1.5 py-0.5 text-sm font-semibold uppercase tracking-wide text-amber-200"
                          style={{
                      ...
                      <div className="mt-2 grid gap-2 md:grid-cols-[160px_1fr]">
                        <label className="block">
                          ...
                          <select
                            ...
                            onChange={e => {
                              setCollateralPolicy(
                                e.target.value as "none" | "selective" | "all"
                              );
                              setStyleApproved(false);
                            }}
                            className="sb-input"
                      ...
                      <div className="mt-2  border border-slate-600 bg-[#081126] p-2">
                        <p className="sb-kicker mb-1.5">
                          Refine from this draft
                        </p>
                        <div className="flex flex-wrap gap-1">
                          {VOICE_FEEDBACK.map(item => (
                            <button
                              ...
                              type="button"
                              onClick={() =>
                                applyVoiceFeedback(item.instruction)
                              }
                              className="sb-btn"
                            >
                              {item.label}
                  ...
                  <div className="flex h-full">
                    ...
                    <div className="sb-surface-body flex-1 space-y-3">
                      <CrmAccountWorkspace
                        ...
                        onStageChange={() => {
                          void authFetch(
                            `/api/crm/accounts?team_id=${encodeURIComponent(teamId)}`
                          ).then(data => {
                            setAccounts(
                              Array.isArray(data) ? (data as Account[]) : []
                            );
                          });
                        }}
                      />
                      <div className=" border border-emerald-500/30 bg-[#081126] p-2">
                        <p className="sb-kicker text-emerald-300">
                          SIGNAL workflow intelligence
                        </p>
                        <p className="mt-1 text-sm font-semibold text-white">
                          {selectedAccount.workflow_intelligence
                            ?.recommended_action ||
                            "Waiting for SIGNAL activity on this account."}
                        </p>
                      ...
                      <div className="mb-2  border border-slate-600 bg-[#081126] p-2">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {(
                            ...
                            <span
                              key={title}
                              className="sb-btn pointer-events-none"
                            >
                              {title}
                            </span>
                          ))}
                        ...
                        <Link
                          href="/sales-console"
                          className="sb-btn sb-btn-ghost mt-2"
                        >
                          Open Sales Console
                        </Link>
                      </div>
                      ...
                      <ol className="mt-1.5 space-y-1.5 text-xs text-slate-400">
                        ...
                        <li>
                          ...
                          check recipient, subject, body.
                        </li>
                        <li>
                          <span className="font-semibold text-slate-100">
                            3. Send approval:
                          </span>{" "}
                          outreach sends after you approve.
                        </li>
                        <li>
                          <span className="font-semibold text-slate-100">
                            4. Reply capture:
                          </span>{" "}
                          replies route back into CRM.
                        </li>
                        <li>
                          <span className="font-semibold text-slate-100">
                            5. Follow-up:
                          </span>{" "}
                          SIGNAL tracks workflow from Profile.
                        </li>
                      </ol>
                        ...
                        <div className="mt-2 border-t border-slate-600 pt-2">
                          ...
                          <div className="mt-1.5 space-y-1.5">
                            {suggestions.map(item => (
                              <div
                                key={item.trigger}
                                className=" border border-slate-600 bg-[#081126] p-2"
                              >
