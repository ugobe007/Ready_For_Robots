...
import { Search, ArrowRight, Zap, Shield, TrendingUp, CheckCircle2, Globe, Target, Users, BarChart3, ChevronDown, Sparkles, FileText, RefreshCw, X, Quote, MessageSquare } from "lucide-react";
...
export default function Home() {
  ...
  const handleScan = (e: React.FormEvent) => {
    e.preventDefault();
    const clean = heroUrl.trim().replace(/^https?:\/\//i, "").replace(/\/+$/, "");
    if (!clean) return;
    navigate(`/results?url=${encodeURIComponent(clean)}`);
  };
  const counter1 = useCountUp(247, 1600);
  const counter2 = useCountUp(38, 1200);
  const counter3 = useCountUp(1204, 1800);
