...
_CLASS_HINTS: list[tuple[re.Pattern[str], str]] = [
    ...
    (re.compile(r"\b(laserweeder|weeding\s+robot|agricultural\s+robot|crop\s+robot|autonomous\s+tractor|combine)\b", re.I), "agriculture"),
    (re.compile(r"\b(hull\s+inspect|underwater\s+robot|marine\s+robot)\b", re.I), "marine"),
    (re.compile(r"\b(drone|uav|evtol|flying\s+car|autonomous\s+aircraft|avionics)\b", re.I), "avionics"),
    (re.compile(r"\b(satellite|orbital\s+debris|space\s+robot|aerospace)\b", re.I), "aerospace"),
    (re.compile(r"\b(construction\s+robot|jobsite\s+robot|layout\s+printer|3d\s+print(?:ed)?\s+home)\b", re.I), "construction"),
]
