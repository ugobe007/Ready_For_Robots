...
const previewLeadPool = [
  {
    ...
    specialReason:
      "Large-scale fulfillment expansion usually creates immediate AMR and picking demand across multiple sites.",
  },
  {
    ...
    specialReason:
      "Sustained labor pressure in high-volume hubs pushes faster buy-cycles for dock and sortation automation.",
  },
  {
    ...
    specialReason:
      "Fresh CapEx signals budgeted automation projects, which usually shortens vendor evaluation timelines.",
  },
  {
    ...
    specialReason:
      "Leadership transitions often reset operations priorities and open new automation initiatives.",
  },
  {
    ...
    specialReason:
      "Live RFP activity means the buying window is active now, not theoretical.",
  },
  {
    ...
    specialReason:
      "Procurement filings are direct evidence that purchasing teams are actively sourcing solutions.",
  },
  {
    ...
    specialReason:
      "Compliance-driven inspection projects usually carry high urgency and executive visibility.",
  },
  {
    ...
    specialReason:
      "Healthcare throughput bottlenecks are expensive, so transport automation often gets prioritized quickly.",
  },
];
...
function robotFitText(lead: HomepageLeadRow): string {
  if (
    Array.isArray(lead.robot_types_needed) &&
    lead.robot_types_needed.length > 0
  ) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="leading-none">
        <div className="text-[13px] font-bold tracking-[.13em] text-white">
          READYFORROBOTS
        </div>
        <div className="mt-1 text-[9px] tracking-[.3em] text-[#58c4ea]">
          SIGNAL / WORKFLOW
        </div>
      </div>
    </div>
...
function StepFrame({
  ...
  return (
    <div className="mx-auto mt-8 w-full max-w-3xl text-center">
      <h2 className="mt-3 text-xl font-semibold tracking-[-0.03em] text-slate-50 sm:text-2xl">
        {title}
      </h2>
...
export default function Home() {
  ...
  const workflowFromQuery =
    params.get("wf") === "buyer"
      ? "looking_for_robots"
      : params.get("wf") === "robot_company"
        ? "robot_company"
        : null;
  const resolvedWorkflow = workflowFromQuery;
  const normalizedUrl = useMemo(
    () => normalizeUrlInput(urlInput || journeyUrl),
    [journeyUrl, urlInput]
  );
  const pageMode: "url" | "identity" | "preview" =
    location === "/journey/identity"
      ? "identity"
      : location === "/journey/preview" || location === "/journey/activate"
        ? "preview"
        : "url";
  ...
  useEffect(() => {
    ...
    void fetch(`${getPublicReadApiBase()}/api/leads/summary?exclude_junk=true`)
      ...
      .then(summary => {
        ...
        const total = Number(
          summary.total ?? summary.companies_in_database ?? 0
        );
  ...
  useEffect(() => {
    ...
    const loadLiveLeads = async () => {
      try {
        const { leads, live } =
          await fetchHomepageLeadPool<HomepageLeadRow>(liveLeadFallback);
        if (cancelled) return;
  ...
  const activateHref = useMemo(() => {
    ...
    nextParams.set(
      "next",
      `/results?url=${encodeURIComponent(normalizedUrl)}&limit=5&src=home_signup_return`
    );
    const resolvedWf =
      resolvedWorkflow === "looking_for_robots" ? "buyer" : "robot_company";
  ...
  const focusHeroInput = () => {
    ...
    const el =
      typeof document !== "undefined"
        ? document.getElementById("hero-company-url")
        : null;
  ...
  const persistWorkflowContext = () => {
    if (typeof window === "undefined" || !normalizedUrl || !resolvedWorkflow)
      return;
    window.sessionStorage.setItem(
  ...
  useEffect(() => {
    if (
      pageMode !== "identity" ||
      !normalizedUrl ||
      analysisProgress < analysisChecklist.length
    )
      return;
    const doneTimer = window.setTimeout(() => {
      const wf =
        resolvedWorkflow === "looking_for_robots" ? "buyer" : "robot_company";
      setLocation(
        `/journey/preview?company_url=${encodeURIComponent(normalizedUrl)}&wf=${wf}`
      );
    }, 480);
  ...
  return (
    <main className="min-h-screen bg-[#081126] text-[#edf4f3]">
      <div className="mx-auto max-w-[1180px] px-5 pb-14 pt-6 sm:pt-10 lg:px-10 lg:pb-28 lg:pt-12">
        ...
        <section
          className={`flex items-center justify-start ${pageMode === "url" ? "min-h-[60vh] sm:min-h-[72vh] lg:min-h-[78vh]" : "min-h-[60vh]"}`}
        >
          <div className="w-full max-w-3xl text-left">
            {pageMode === "url" && (
              <div className="w-full max-w-3xl text-left">
                ...
                <h1
                  className="mt-4 text-[clamp(2.1rem,8.4vw,6.3rem)] font-semibold leading-[0.9] tracking-[-0.045em] text-slate-50 sm:mt-5"
                  style={{ textShadow: "0 10px 34px rgba(5, 10, 20, 0.48)" }}
                >
                ...
                <div className="mt-7 w-full max-w-[780px] border-b border-emerald-400/45 pb-3 sm:mt-10 lg:mt-11">
                  <label
                    htmlFor="hero-company-url"
                    className="mb-2 block text-left text-xs font-semibold uppercase tracking-[0.18em] text-[#8ec8b9]"
                  >
                ...
                <p className="mt-2 text-[11px] font-medium uppercase tracking-[0.14em] text-slate-500">
                  1. Submit URL · 2.{" "}
                  {session?.access_token ? "15 sales leads" : "5 sales leads"} ·
                  3.{" "}
                  {session?.access_token ? "Copy outreach" : "Sign up for 15"}
                </p>
              </div>
            )}
            ...
            {pageMode === "identity" && (
              <StepFrame
                title="Finding customers for your team..."
                copy="Give us a moment to map your best buyers and next outreach angle."
              >
                  ...
                  <div className="space-y-2">
                    {analysisChecklist.map((item, index) => {
                      ...
                      return (
                        <div
                          key={item}
                          className={`flex items-center gap-2 text-sm ${done ? "text-emerald-200" : "text-slate-400"}`}
                        >
                          <CheckCircle2
                            className={`h-4 w-4 ${done ? "text-emerald-300" : "text-slate-500"}`}
                          />
                          <span>{item}</span>
                        </div>
                      );
              ...
              >
                <div className="mx-auto grid max-w-5xl gap-4 lg:grid-cols-3">
                  {previewLeads.slice(0, 3).map(lead => {
                    ...
                    return (
                      <article
                        key={`${lead.company}-${lead.signal}`}
                        className="rounded-2xl border border-emerald-400/45 bg-[#0b162f] p-5 text-left shadow-[0_20px_45px_-25px_rgba(0,200,150,0.8)]"
                      >
                        ...
                        <ul className="mt-2 space-y-1.5 text-sm text-slate-200">
                          ...
                          <li className="flex items-start gap-2">
                            <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 text-emerald-300" />
                            New automation hiring indicators detected
                          </li>
                          <li className="flex items-start gap-2">
                            <CheckCircle2 className="mt-0.5 h-3.5 w-3.5 text-emerald-300" />
                            Robot fit: {lead.fit}
                ...
                <p className="mt-5 text-lg font-semibold text-emerald-200">
                  We found {unlockMoreCount.toLocaleString()} more buyers in the
                  pipeline. Unlock them.
                </p>
                <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
                  <button
                    type="button"
                    onClick={() =>
                      setPreviewLeadOffset(
                        prev => (prev + 1) % previewLeadPool.length
                      )
                    }
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 rounded-md bg-[#00c896] px-5 py-2.5 text-sm font-semibold text-[#06261f] transition hover:bg-[#00d9a3]"
                  >
            ...
            <div className="mt-8 text-xs text-[#7fa2c8] sm:mt-12">
              ...
              <Link
                href="/login"
                className="font-semibold text-[#9fcaef] hover:text-white"
              >
                Sign in
              </Link>
            </div>
        ...
        {pageMode === "url" && (
          <>
            <section className="mt-10 border-t border-slate-800/70 pt-12 sm:pt-14">
              ...
              <p className="mt-3 text-[15px] text-slate-300 sm:text-[16px]">
                ReadyForRobots detects the signals. You get the opportunity.
              </p>
              ...
              <div className="mt-8 border-y border-emerald-400/35 bg-[#061124] sm:mt-9">
                <div className="flex items-center justify-between border-b border-emerald-900/45 py-1.5 font-mono text-[10px] uppercase tracking-[0.2em] text-[#87dbca]">
                  ...
                  <span className="inline-flex items-center gap-2 text-[#8ce6d2]">
                    <span
                      className={`h-2 w-2 rounded-full ${isLiveFeed ? "bg-[#22d3a7] animate-pulse" : "bg-amber-300"}`}
                    />
                ...
                <div className="font-mono">
                  {rotatingSignalRows.map((row, index) => (
                    <article
                      key={row.company}
                      className="border-b border-[#16304b] py-2 last:border-b-0"
                    >
                      <div className="flex items-baseline justify-between gap-4">
                        <h3 className="text-[14px] font-semibold tracking-[0.06em] text-emerald-300 sm:text-[15px]">
                          {row.company}
                        </h3>
                        <p
                          className={`text-[11px] font-semibold tracking-[0.14em] ${row.heat === "HOT" ? "text-[#f59e0b]" : "text-[#7dd3fc]"}`}
                        >
                          {row.heat === "HOT" ? "HOT" : "WARM"}
                        </p>
                      </div>
                      <p className="mt-1 truncate text-[12px] leading-5 text-slate-300">
                        {row.summary}
                      </p>
                      <p className="mt-0.5 text-[12px] leading-5 text-[#a7c7de]">
                        Robot Fit: {row.robotFit} · Why Now: {row.whyNow} ·
                        Decision Makers: {row.buyers}
                      </p>
                      {index < rotatingSignalRows.length - 1 ? (
                        <div
                          className="mt-2 h-px w-full bg-gradient-to-r from-transparent via-[#2a455d] to-transparent"
                          aria-hidden
                        />
                      ) : null}
                    </article>
              ...
              <div className="mt-8">
                <Link
                  href="/signals"
                  className="inline-flex items-center gap-2 text-base font-semibold text-[#85e8ce] transition hover:text-[#b5f7e4]"
                >
                ...
                {!isLiveFeed ? (
                  <p className="mt-2 text-xs text-amber-200/90">
                    Live API unavailable right now. Showing fallback examples
            ...
            <section className="mt-16 border-t border-slate-800/70 pt-14 sm:mt-20 sm:pt-16">
              ...
              <p className="mt-4 max-w-4xl text-[15px] leading-7 text-slate-300 sm:text-[16px]">
                Most robot sales teams start with a list of companies and try to
                figure out who might need automation.
              </p>
              <p className="mt-3 text-base font-semibold text-[#9af2dc] sm:text-lg">
                ReadyForRobots starts with the need.
              </p>
              ...
              <div className="mt-10 space-y-5">
                <div className="rounded-xl border border-red-300/25 bg-[#130f14] px-5 py-4">
                  ...
                  <p className="mt-2 font-mono text-sm leading-8 text-red-100/85">
                    Company -&gt; Research -&gt; Cold Call -&gt; Follow Up -&gt;
                    Wait -&gt; Maybe
                  </p>
                </div>
                <div className="rounded-xl border border-emerald-300/30 bg-[#071a19] px-5 py-4">
                  ...
                  <p className="mt-2 font-mono text-sm leading-8 text-emerald-100">
                    Need Detected -&gt; Robot Matched -&gt; Buyer Identified
                    -&gt; Sell
                  </p>
                </div>
              </div>
