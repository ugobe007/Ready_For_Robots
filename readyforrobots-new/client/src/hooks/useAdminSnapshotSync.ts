...
const isTransientSnapshotStatus = (status: number) => [408, 429, 500, 502, 503, 504].includes(status);
...
export function useAdminSnapshotSync(
  ...
  const sectionInFlightRef = useRef<Map<AdminSectionName, Promise<void>>>(new Map());
  const sectionCooldownUntilRef = useRef<Map<AdminSectionName, number>>(new Map());
  const sectionBreakerOpenUntilRef = useRef<Map<AdminSectionName, number>>(new Map());
  ...
  const publishTelemetry = useCallback(() => {
    ...
    const now = Date.now();
    const activeCooldownSections = Array.from(sectionCooldownUntilRef.current.entries())
      .filter(([, until]) => until > now)
      .map(([section]) => section);
    const activeBreakerSections = Array.from(sectionBreakerOpenUntilRef.current.entries())
      .filter(([, until]) => until > now)
      .map(([section]) => section);
    (window as Window & { __rfrAdminSnapshotTelemetry?: Record<string, unknown> }).__rfrAdminSnapshotTelemetry = {
  ...
  const refreshSection = useCallback(async (section: AdminSectionName, force = false) => {
    ...
    const now = Date.now();
    const breakerOpenUntil = sectionBreakerOpenUntilRef.current.get(section) ?? 0;
    ...
    const run = (async () => {
      try {
        ...
        while (attempts < 2) {
          attempts += 1;
          res = await adminFetch(`/api/admin/snapshot/section/${section}?${params.toString()}`);
          if (res.status === 304) {
        ...
        if (!res || !res.ok) {
          ...
          throw new Error(`snapshot section ${section} failed (${status || "network"})`);
        }
        ...
        sectionCooldownUntilRef.current.set(section, Date.now() + SNAPSHOT_SECTION_COOLDOWN_MS);
        if (streak >= SNAPSHOT_SECTION_BREAKER_FAIL_STREAK) {
          sectionBreakerOpenUntilRef.current.set(section, Date.now() + SNAPSHOT_SECTION_BREAKER_OPEN_MS);
          sectionFailStreakRef.current.set(section, 0);
          telemetryRef.current.breakerOpenCount += 1;
        }
        telemetryRef.current.failures += 1;
        telemetryRef.current.lastError = err instanceof Error ? err.message : "snapshot section failed";
        publishTelemetry();
