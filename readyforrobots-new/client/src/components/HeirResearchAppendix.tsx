      ...
      <ReportDetails
        summary={
          ...
          >
            ...
            <p className="mt-1 text-[13px] leading-snug max-w-2xl" style={{ color: RR.textMuted }}>
              Demo culture vs deployment reality — HEIF scores and vendor analysis in the PDF.
            </p>
          </div>
        }
      >
        ...
        <ul className="mb-4 space-y-2">
          {HEIR_PULL_QUOTES.map((q) => (
            <li key={q} className="flex gap-2 text-[13px] leading-snug" style={{ color: RR.textMuted }}>
              <span className="shrink-0" style={{ color: RR.purple }}>—</span>
        ...
        <ReportTable
          ...
          rows={HEIF_BENCHMARK.map((row) => [
            <span className="font-semibold" style={{ color: RR.text }}>{row.company}</span>,
        ...
        <p className="mt-2 text-[10px]" style={{ color: RR.textDim }}>
          HEIF snapshot for seven vendors from HEIR 2026. The live index applies the same framework to all ranked robots.
        </p>
      </ReportDetails>
    </section>
  );
