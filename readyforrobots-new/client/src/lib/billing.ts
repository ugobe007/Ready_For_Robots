...
export async function fetchBillingConfig(): Promise<BillingConfig> {
  const res = await fetch(`${getApiBase()}/api/billing/config`, liveFetchInit());
  if (!res.ok) return { enabled: false };
  return res.json();
}

export async function startCheckout(accessToken: string, tier: "pro" | "premium"): Promise<string> {
  const res = await fetch(`${getApiBase()}/api/billing/checkout`, liveFetchInit({
    method: "POST",
...
export async function syncCheckoutSession(accessToken: string, sessionId: string): Promise<void> {
...
export async function openBillingPortal(accessToken: string): Promise<string> {
  const res = await fetch(`${getApiBase()}/api/billing/portal`, liveFetchInit({
    method: "POST",
    headers: authHeader(accessToken),
  }));
  const data = await res.json().catch(() => ({}));
  if (!res.ok) throw new Error(String(data.detail || "Could not open billing portal"));
