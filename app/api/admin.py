...
class PurgeJunkPayload(BaseModel):
    dry_run: bool = True   # default safe: preview only, no deletes
    limit: Optional[int] = None   # cap how many to delete in one pass


@router.post("/purge-junk")
def purge_junk(payload: PurgeJunkPayload, db: Session = Depends(get_db)):
    """
    Scan all company records and delete those whose name is flagged by is_junk().
    dry_run=true (default): returns a preview without deleting anything.
    dry_run=false:          deletes junk companies + their signals (CASCADE).
    """
    from app.services.lead_filter import is_junk
    ...
    for c in companies:
        ...
        if bad:
            junk_found.append({
                ...
                "name":   c.name,
                "reason": reason,
                "signals": len(c.signals) if hasattr(c, "signals") else 0,
            })

    if payload.limit:
        junk_found = junk_found[: payload.limit]

    preview = junk_found[:100]  # always cap what we return
    ...
    return {
        ...
        "sample_deleted": [r["name"] for r in junk_found[:20]],
    }
