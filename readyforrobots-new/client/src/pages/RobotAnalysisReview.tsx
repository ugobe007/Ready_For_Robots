...
export default function RobotAnalysisReview() {
    ...
    const poll = async () => {
      try {
        ...
        const res = await fetch(
          `${getApiBase()}/api/v1/robot-analyses/${analysisId}`,
          { headers }
        );
        if (!res.ok) {
          const body = await res.json().catch(() => null);
          throw new Error(
            body?.error?.message || `Failed to load analysis (${res.status})`
          );
        }
        ...
        if (
          !cancelled &&
          ["queued", "crawling", "extracting"].includes(body.status)
        ) {
  ...
  const confirm = async () => {
    ...
    try {
      ...
      if (token) headers["X-Analysis-Token"] = token;
      const res = await fetch(
        `${getApiBase()}/api/v1/robot-analyses/${analysisId}/confirm`,
        {
          method: "POST",
          headers,
          body: JSON.stringify({
      ...
      if (!res.ok) {
        throw new Error(
          body?.error?.message || `Confirm failed (${res.status})`
        );
      }
      setLocation(
        `/robots/${body.robot_id}/review?profile=${body.profile_version_id}`
      );
    } catch (err) {
    ...
    <main className="min-h-screen bg-[#081126] text-[#edf4f3]">
      <div className="mx-auto max-w-3xl px-5 py-10">
        ...
        <p className="mt-2 text-sm text-slate-300">
          {analysis?.draft_profile?.manufacturer || "Manufacturer pending"} ·{" "}
          {analysis?.status}
        ...
        {analysis?.message && (
          <p className="mt-2 text-sm text-[#8ec8b9]">{analysis.message}</p>
        )}
        {error && (
          ...
          <section className="mt-10 space-y-8">
            ...
            <div className="flex flex-wrap items-center gap-4">
              ...
              <p className="text-xs text-slate-400">
                Confidence{" "}
                {Math.round((analysis.draft_profile.confidence || 0) * 100)}%.
                Unknown stays unknown — never invented as false.
              </p>
            </div>
          </section>
        )}
