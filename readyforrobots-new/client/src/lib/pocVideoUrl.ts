...
export const JOBS_POC_VIDEO_SCRIPT_HEADING = "Record this Job Card, not a brand reel";
...
export function parsePocVideoUrl(raw: string | null | undefined): PocVideoParsed | null {
  const text = (raw || "").trim();
  if (!text) return null;
...
export function pocVideoUrlIssue(raw: string | null | undefined): string | null {
  const text = (raw || "").trim();
  if (!text) return null;
  try {
...
export function pocVideoScriptBeats(opts: {
  ...
  const skuLine = models.length ? models.join(", ") : "the catalogued SKU you will use";
