...
def apply_to_job(
    ...
    *,
    ...
    contacts = employer_contacts_from_job(payload if isinstance(payload, dict) else {})
    if kept and kept.employer_email:
        hit = normalize_recipient_email(kept.employer_email)
        if hit and not any(c.get("email") == hit for c in contacts):
            contacts.append({"email": hit, "source": "job_card"})
    email = contacts[0]["email"] if contacts else None
    robot = (robot_name or (kept.robot_name if kept else "") or "this robot").strip()
    oem_company = (company_name or (kept.robot_name if kept else "") or robot).strip()
    sku = models[0] if models else robot
    video_info = resolve_apply_video(
        pasted_url=poc_video_url,
        company=oem_company,
        sku=sku,
        robot=robot,
    )
    ...
    snapshot = {
        ...
        "contacts": contacts,
        "why": why_text,
        "clip_description": video_info.get("clip_description"),
        "video_search_url": video_info.get("video_search_url") or "",
        "video_note": video_info.get("video_note") or "",
    }
    ...
    send_error = None
    if send:
        application.status = STATUS_APPLIED
        if not email:
        ...
        application.status = STATUS_PREPARED
        application.send_status = SEND_PREPARED
        application.thread_state = THREAD_DRAFT
        if not email:
            application.send_error = _NO_EMAIL_REASON
        record_activity(
...
def apply_selected_jobs(
    ...
    *,
    ...
    """Prepare the same offer on every selected Job Card. Operator sends."""
...
def send_prepared_application(
    ...
    """Operator-confirmed send. Never auto-emails on prepare."""
    row = get_application(db, user, application_id)
    if row.send_status == SEND_SENT:
    ...
    attached = documents_for_application(db, row.id)
    try:
        from app.services.resend_email import ResendEmailError, send_email_via_resend
        ...
        result = send_email_via_resend(
            ...
            attachments=resend_attachments_for(attached) or None,
            idempotency_key=f"jobs-crm-apply/{row.id}",
        )
    except Exception as exc:
        from app.services.resend_email import ResendEmailError
        ...
        db.commit()
        raise ValueError(send_error) from exc
    row.send_status = SEND_SENT
    row.send_error = None
    row.resend_id = result.get("resend_id")
    ...
    notify_oem_status(db, user, row, "applied")
    db.commit()
    return application_with_thread(db, user, application_id)
...
def application_payload(row: JobApplication, *, include_messages: bool = False) -> dict[str, Any]:
    payload = {
        ...
        "can_operator_send": bool(row.employer_email)
        and (getattr(row, "status", None) or "") == "prepared"
        and (row.send_status or "") == SEND_PREPARED,
        "no_email_reason": None if row.employer_email else _NO_EMAIL_REASON,
        "status": getattr(row, "status", None) or "applied",
        "draft": (row.offer_snapshot or {}).get("draft"),
        "why": (row.offer_snapshot or {}).get("why"),
        "contacts": (row.offer_snapshot or {}).get("contacts") or [],
        "clip_description": (row.offer_snapshot or {}).get("clip_description"),
