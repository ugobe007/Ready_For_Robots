...
type OutreachHistoryItem = NonNullable<
  SupplyCompany["outreach_history"]
>[number];

const LIVE_SENT_STATUSES = new Set([
  "sent",
  "delivered",
  "opened",
  "clicked",
...
function initialDraft(row: SupplyCompany): DraftState {
  ...
  const contact = recommended.length
    ? recommended.join(", ")
    : row.contact_strategy.primary?.contact ||
      row.robot_company.contact_email ||
      "";
...
export default function SupplyPipeline() {
  ...
  const loadPipeline = useCallback(async () => {
    ...
    try {
      ...
      setDrafts(
        Object.fromEntries(
          companies.map((row: SupplyCompany) => [
            row.robot_company.id,
            initialDraft(row),
          ])
        )
      );
      ...
      setErr(
        aborted
          ...
          : e instanceof Error
            ? e.message
            : "Could not load supply pipeline"
      );
    } finally {
      clearTimeout(timer);
      setLoading(false);
  ...
  const selected =
    selectedId == null
      ? null
      : (rows.find(row => row.robot_company.id === selectedId) ?? null);
  const selectedDraft = selected ? drafts[selected.robot_company.id] : null;
  const approvedCount = Object.values(drafts).filter(
    draft => draft.approved && !draft.sent
  ).length;
  const sentCount = Object.values(drafts).filter(draft => draft.sent).length;
  const needsActionCount = Object.values(drafts).filter(
    draft => !draft.sent
  ).length;
      ...
      const response = await fetch(
        ...
        liveFetchInit({
          ...
          body: JSON.stringify({
            ...
            payload: {
              operator_checkpoint:
                "Approved from supply pipeline review console",
              buyer_matches: row.lead_matches
                .slice(0, 3)
  ...
  const validateDraftCompanyCopy = (row: SupplyCompany, draft: DraftState) => {
    ...
    if (!draft.body.includes(row.robot_company.company_name)) {
      toast.error(
        `Draft body mismatch. It must mention ${row.robot_company.company_name}.`
      );
      return false;
    }
    return true;
  ...
  const sendOne = async (row: SupplyCompany, test = false) => {
    ...
    if (!test && !session?.access_token) {
      toast.error(
        "Sign in before sending live outreach so SIGNAL can copy the message to your CRM."
      );
      return;
    }
    patchDraft(id, { sending: true });
    try {
      ...
      patchDraft(id, {
        ...
        crmOutreachMessageId:
          result.crm_outreach_message_id || draft.crmOutreachMessageId,
      ...
      toast.success(
        test
          ? `Test email sent to ${signedInEmail}.`
          : `Sent outreach to ${row.robot_company.company_name} and copied it to CRM.`
      );
    } catch (e) {
      patchDraft(id, { sending: false });
      ...
      <main className="admin-workspace flex-1 px-4 pb-12 pt-24">
        <div className="mx-auto max-w-6xl">
          ...
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-gray-500">
            SIGNAL researches robot companies, identifies who to contact, shows
            three matched buyer leads, and drafts a signup plus meeting email
            for review.
          </p>
          <div className="mt-5 flex flex-wrap items-center gap-2 rounded-2xl border border-gray-200 bg-white p-3">
            ...
            <p className="text-[11px] text-gray-400">
              Operator controls: review, edit, approve, then send. Nothing sends
              without approval.
            </p>
          </div>
          {err && (
          ...
          {loading && !err && (
            <p className="mt-4 rounded-lg border border-gray-200 bg-white p-3 text-sm text-gray-500">
              SIGNAL is researching robot companies and matching buyer leads —
            ...
            <aside className="rounded-2xl border border-gray-200 bg-white">
              <div className="border-b border-gray-100 px-4 py-3">
                ...
                <p className="mt-1 text-[11px] text-gray-400">
                  Unsent prospects are separated from companies SIGNAL already
                  contacted.
                </p>
                <div className="mt-3 grid grid-cols-3 gap-1 rounded-xl border border-gray-300 bg-gray-100 p-1">
                  {[
                    ...
                    <button
                      ...
                      onClick={() =>
                        setSidebarView(
                          item.key as "needs_action" | "sent" | "all"
                        )
                      }
              ...
              <div className="max-h-[680px] overflow-y-auto p-2">
                {visibleRows.length === 0 && (
                  <div className="m-2 rounded-xl border border-dashed border-gray-200 p-4 text-center">
                    ...
                    <p className="mt-1 text-[11px] text-gray-400">
                      Switch tabs above to see sent or all companies.
                    </p>
                  </div>
                )}
                {visibleRows.map(row => {
                  ...
                  const liveHistory = latestLiveHistory(row);
                  const sentLabel = liveHistory
                    ? deliveryLabel(liveHistory)
                    : "Sent";
                  return (
                    <button
                      key={company.id}
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        >
                          {Math.round(
                            company.vendor_list_score ?? company.lead_score ?? 0
                          )}
                        </span>
                      </div>
                      <p className="mt-1 truncate text-[11px] text-gray-400">
                        {company.robot_type || "robotics"} ·{" "}
                        {company.target_market || "market TBD"}
                      </p>
            ...
            {selected && selectedDraft && (
              <section className="grid gap-4">
                <div className="rounded-2xl border border-gray-200 bg-white p-5">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    <div>
                      <h2 className="text-xl font-black text-gray-900">
                        {selected.robot_company.company_name}
                      </h2>
                      <p className="mt-1 text-sm text-gray-600">
                        {selected.robot_company.robot_type || "Robotics"} for{" "}
                        {selected.robot_company.target_market ||
                          "target market review"}
                      </p>
                    </div>
                    {selected.robot_company.website && (
                      <a
                        href={selected.robot_company.website}
                        target="_blank"
                        rel="noreferrer"
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    <div className="rounded-xl border border-gray-100 bg-white p-3">
                      ...
                      <p className="mt-1 text-sm font-bold text-gray-800">
                        {selected.contact_strategy.primary?.role ||
                          "Partnerships"}
                      </p>
                      <p className="mt-1 break-all text-xs text-gray-500">
                        {selected.contact_strategy.primary?.contact ||
                          selected.robot_company.contact_email ||
                          "Research contact first"}
                      </p>
                      {selected.contact_strategy.primary
                        ?.needs_verification && (
                    ...
                    <div className="rounded-xl border border-gray-100 bg-white p-3 md:col-span-2">
                      ...
                      <p className="mt-2 text-[10px] leading-relaxed text-gray-400">
                        ...
                        {(
                          selected.contact_strategy.communication_policy
                            ?.role_inboxes || []
                        ).join(", ") || "after domain research"}
                        . Named decision makers use first.last, first initial +
                        last, last, and first-name patterns.
                      </p>
                      <p className="mt-2 text-[10px] font-bold uppercase tracking-widest text-gray-400">
                        Research status:{" "}
                        {selected.contact_research?.status ||
                          selected.contact_strategy.communication_policy
                            ?.research_status ||
                          "not run"}
                      </p>
                  ...
                  {!!(selected.contact_research?.decision_makers || [])
                    .length && (
                    <div className="mt-3 rounded-xl border border-emerald-400/15 bg-emerald-400/5 p-3">
                      <p className="text-[10px] font-semibold uppercase tracking-widest text-emerald-900">
                        Researched decision makers
                      </p>
                      <div className="mt-2 grid gap-2 md:grid-cols-3">
                        {(selected.contact_research?.decision_makers || []).map(
                          person => (
                            <div
                              key={`${person.first_name}-${person.last_name}-${person.source_url}`}
                            ...
                            >
                              ...
                              {person.source_url && (
                                <a
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="mt-1 block truncate text-[10px] text-emerald-200/70 underline"
                                >
                                  Source
                                </a>
                              )}
                  ...
                  {(selected.contact_strategy.targets || []).length > 1 && (
                    <div className="mt-3 rounded-xl border border-gray-300 bg-gray-50 p-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {(selected.contact_strategy.targets || []).map(
                          target => (
                            <button
                              ...
                              onClick={() =>
                                target.contact &&
                                patchDraft(selected.robot_company.id, {
                                  to: target.contact,
                                  approved: false,
                                })
                              }
                      ...
                      {!!selected.contact_strategy.recommended_to?.length && (
                        <button
                          ...
                          onClick={() =>
                            patchDraft(selected.robot_company.id, {
                              to:
                                selected.contact_strategy.recommended_to?.join(
                                  ", "
                                ) || "",
                              approved: false,
                            })
                  ...
                  <div className="mt-3 rounded-xl border border-gray-300 bg-gray-50 p-3">
                    <button
                      ...
                      onClick={() =>
                        patchDraft(selected.robot_company.id, {
                          expanded: !selectedDraft.expanded,
                        })
                      }
                    ...
                    {selectedDraft.expanded && (
                      <div className="mt-3 grid gap-2 text-xs text-gray-500 md:grid-cols-2">
                        <p>
                          ...
                          {selected.robot_company.website || "Unknown"}
                        </p>
                        <p>
                          ...
                          {(
                            selected.contact_strategy.recommended_to || []
                          ).join(", ") || "Research needed"}
                        </p>
                        <p>
                          <span className="text-gray-600">Research pages:</span>{" "}
                          {(selected.contact_research?.sources || []).length ||
                            0}
                        </p>
                        <p>
                          <span className="text-gray-600">LinkedIn links:</span>{" "}
                          {(selected.contact_research?.linkedin_urls || [])
                            .length || 0}
                        </p>
                        <p>
                          <span className="text-gray-600">Tracking ID:</span>{" "}
                          {selectedDraft.trackingId || "Not tracked yet"}
                        </p>
                        <p>
                          <span className="text-gray-600">Reply path:</span>{" "}
                          {selectedDraft.replyTo || "Created on approval/send"}
                        </p>
                        <p>
                          <span className="text-gray-600">Robot type:</span>{" "}
                          {selected.robot_company.robot_type || "Unknown"}
                        </p>
                        <p>
                          <span className="text-gray-600">Target market:</span>{" "}
                          {selected.robot_company.target_market || "Unknown"}
                        </p>
                        <p>
                          <span className="text-gray-600">Lead score:</span>{" "}
                          {selected.robot_company.lead_score ?? "Unknown"}
                        </p>
                        <p>
                          <span className="text-gray-600">Vendor score:</span>{" "}
                          {selected.robot_company.vendor_list_score ??
                            "Unknown"}
                        </p>
                      </div>
                    )}
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5">
                  <p className="text-[10px] uppercase tracking-widest text-gray-400">
                    3 buyer lead matches for email
                  </p>
                  <div className="mt-3 grid gap-3 md:grid-cols-3">
                    {selected.lead_matches.slice(0, 3).map(lead => (
                      ...
                      >
                        <div className="flex items-center justify-between gap-2">
                          <p className="truncate text-sm font-bold text-gray-900/82">
                            {lead.company_name}
                          </p>
                        ...
                        <p className="mt-2 text-[11px] leading-relaxed text-gray-500">
                          {cleanAndClampText(lead.why_match, 150)}
                        </p>
                        <p className="mt-2 text-[10px] leading-relaxed text-gray-400">
                          {cleanAndClampText(lead.signal, 130)}
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <div className="rounded-2xl border border-gray-200 bg-white p-5">
                  ...
                  {!!(selected.outreach_history || []).length && (
                    <div className="mb-4 rounded-xl border border-gray-100 bg-black/10 p-3">
                      <p className="text-[10px] uppercase tracking-widest text-gray-400">
                        Tracked outreach history
                      </p>
                      <div className="mt-2 grid gap-2">
                        {(selected.outreach_history || [])
                          .slice(0, 3)
                          .map(item => (
                            <div
                              key={item.id}
                              className="rounded-lg border border-gray-100 bg-white p-2 text-[11px] text-gray-500"
                            >
                              <div className="flex flex-wrap items-center justify-between gap-2">
                                <span className="font-bold text-gray-600">
                                  {item.is_test ? "Test" : "Live"} ·{" "}
                                  {deliveryLabel(item)}
                                </span>
                                <span>
                                  {item.sent_at ||
                              ...
                              <p className="mt-1 truncate">
                                To: {(item.to_emails || []).join(", ")}
                              </p>
                              {(item.resend_id || item.reply_to) && (
                                <p className="mt-1 truncate text-gray-400">
                                  Resend: {item.resend_id || "pending"} · Reply:{" "}
                                  {item.reply_to || "not set"}
                                </p>
                              )}
                              {(item.problem_reason ||
                                item.cal_delivery_action) && (
                  ...
                  <div className="grid gap-3">
                    <label className="block">
                      ...
                      <span className="mt-1 block text-[10px] text-gray-400">
                        Separate multiple recipients with commas. Inferred role
                        inboxes and decision-maker patterns should be verified
                        before live send.
                      </span>
                    </label>
                    <label className="block">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    ...
                    <div
                      ...
                      title={
                        selectedDraft.sent
                          ? "Outreach sent and copied to CRM sent messages."
                          : "Live send will record this in CRM sent messages."
                      }
                    >
                      <span
                        className={`flex h-4 w-4 items-center justify-center rounded border ${
                          selectedDraft.sent
                            ? "border-emerald-300 bg-emerald-400 text-[#111827]"
                            : "border-gray-200"
                        }`}
                      >
                        {selectedDraft.sent ? "✓" : ""}
                  ...
                  {selectedDraft.sent && (
                    <div className="mt-3 rounded-xl border border-emerald-300 bg-emerald-50 p-3 text-xs font-medium text-emerald-950">
                      <p className="font-bold text-emerald-900">
                        Workflow checkpoint complete: email sent and activity
                        copied to CRM sent messages.
                      </p>
                      <p className="mt-1 text-emerald-800">
                        CRM account: {selectedDraft.crmAccountId || "tracked"} ·
                        Message:{" "}
                        {selectedDraft.crmOutreachMessageId ||
                  ...
                  <div className="mt-4 grid gap-2 md:grid-cols-2">
                    <p className="rounded-xl border border-gray-100 bg-white p-3 text-xs text-gray-500">
                      {selected.cta.signup}
                    </p>
                    <p className="rounded-xl border border-gray-100 bg-white p-3 text-xs text-gray-500">
                      {selected.cta.meeting}
                    </p>
                  </div>
                </div>
              </section>
