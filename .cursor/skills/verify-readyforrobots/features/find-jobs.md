...
- **Open FIND.** Load `/?visit=jobs` (or `/` then Jobs for Robots). Heading is `Find Jobs for Robots.` URL placeholder is `Paste robot product URL`. Form name is `Find jobs for your robot`.
