...
const RADAR_SIGNAL_ROWS = [
  {
    ...
    track: "Sales" as OpportunityTrack,
    baseline: 0.82,
  },
  {
  ...
  {
    label: "Contract Win",
    track: "Partnership" as OpportunityTrack,
    baseline: 0.54,
  },
  {
    label: "Integrator Fit",
    track: "Partnership" as OpportunityTrack,
    baseline: 0.5,
  },
  {
    label: "Industry News Trigger",
    track: "Partnership" as OpportunityTrack,
    baseline: 0.46,
  },
];
...
const SIGNAL_TYPES = [
  {
    ...
    description:
      "Companies unable to fill critical roles — the most reliable indicator of automation readiness.",
    sources: [
      "Job boards",
      "Earnings calls",
    ...
    example:
      '"We cannot staff overnight shifts" — Q3 earnings call, Silver Peak Hospitality',
    frequency: "Daily",
    avgScore: 88,
  },
  {
    ...
    category: "Growth",
    description:
      "New facilities, new markets, or significant headcount growth — companies building capacity need automation baked in.",
    sources: [
      "Press releases",
      "Real estate permits",
    ...
    example:
      '"Opening 2 new distribution centers in Q1 2027" — press release, DesertLine Logistics',
    frequency: "Daily",
    avgScore: 84,
  },
  {
    ...
    category: "Operational",
    description:
      "OSHA filings, injury reports, and safety incidents indicate high-risk manual tasks ready for automation.",
    sources: [
      "OSHA database",
    ...
    example:
      "3 repetitive strain injury filings in 6 months — OSHA records, Apex Manufacturing",
    frequency: "Weekly",
    avgScore: 81,
  },
  {
    ...
    category: "Financial",
    description:
      "Capital expenditure announcements signal budget allocation for equipment and infrastructure.",
    sources: [
      "Earnings calls",
      "SEC filings",
      "Press releases",
    ...
    example:
      '"$12M facility upgrade approved" — earnings call, Harbor Fresh Foods',
    frequency: "Weekly",
    avgScore: 86,
  },
  {
    ...
    category: "Intent",
    description:
      "Job postings for automation engineers, robotics technicians, or process improvement roles signal active evaluation.",
    sources: ["LinkedIn", "Indeed", "Glassdoor", "Company career pages"],
    industries: ["All industries"],
    example:
      '"Automation Engineer" job posting + "Process Improvement Manager" — DesertLine Logistics',
    frequency: "Daily",
    avgScore: 79,
  },
  {
    ...
    category: "External",
    description:
      "Regulatory changes, competitor moves, or industry events that create urgency for automation adoption.",
    sources: [
    ...
    example:
      "New FDA food safety regulations requiring traceability — triggers food processing automation review",
    frequency: "Weekly",
    avgScore: 72,
  },
  {
    ...
    category: "Intent",
    description:
      "New COO, VP of Operations, or Head of Manufacturing often signals a mandate to modernize operations.",
    sources: ["LinkedIn", "Press releases", "Company announcements"],
    industries: ["All industries"],
    example:
      "New COO hired from Amazon — known for automation-first operations philosophy",
    frequency: "Weekly",
    avgScore: 77,
  },
  {
    ...
    category: "Growth",
    description:
      "Major contract wins create immediate capacity pressure — the perfect moment to propose automation.",
    sources: [
      "Press releases",
    ...
    example:
      "Won 5-year DoD logistics contract requiring 3x throughput increase",
    frequency: "Weekly",
    avgScore: 83,
  },
];
...
const fallbackLiveSignals: LiveOpportunitySignal[] = [
  {
    ...
    text: "40% housekeeping vacancy and overnight staffing pressure.",
  },
  {
    id: "desertline",
    ...
    score: 88,
    time: "18m ago",
    color: MARKET_COLORS.emerald,
    track: "Sales",
    industry: "Logistics",
    text: "Opening two distribution centers with throughput pressure.",
  },
  {
    id: "apex",
    ...
    text: "Manual handling injuries indicate a strong automation case.",
  },
  {
    id: "harbor",
    company: "Harbor Fresh Foods",
    type: "CapEx Announcement",
    score: 85,
    time: "3h ago",
    color: MARKET_COLORS.amber,
    track: "Sales",
    industry: "Food Processing",
    text: "$12M facility upgrade creates budget timing.",
  },
  {
    id: "ridgeline",
    company: "Ridgeline Hotels",
    type: "Integrator Fit",
    score: 91,
    time: "5h ago",
    color: MARKET_COLORS.emeraldBright,
    track: "Partnership",
    industry: "Hospitality",
    text: "Regional operating footprint maps to channel partner coverage.",
  },
  {
    id: "cascade",
    company: "Cascade Fulfillment",
    type: "Automation Hiring",
    score: 76,
    time: "8h ago",
    color: MARKET_COLORS.amberBright,
    track: "Partnership",
    industry: "Logistics",
    text: "Automation engineer hiring suggests active vendor evaluation.",
  },
];
...
function opportunityTrack(lead: ApiLead): OpportunityTrack {
  const text = [
    ...
    .join(" ")
    .toLowerCase();
  return /(partner|partnership|channel|integrator|distributor|co-sell|alliance)/.test(
    text
  )
    ? "Partnership"
    : "Sales";
...
function mapLeadToLiveSignal(
  lead: ApiLead,
  index: number
): LiveOpportunitySignal {
  const firstSignal = lead.signals?.[0];
  const type =
    firstSignal?.signal_label ||
    titleize(firstSignal?.signal_type || "demand_signal");
  ...
  return RADAR_SIGNAL_ROWS.map((seed, index) => {
    ...
    const value = Math.min(
      ...
      Math.max(
        0.22,
        average / 100 +
          (matched ? Math.min(matched.count * 0.02, 0.08) : 0) +
          pulse
      )
    );
    return {
      ...
      delta: `+${Math.max(
        1,
        (matched?.count || 0) + Math.round((average - 58) / 12)
      )
        .toString()
        .padStart(2, "0")}`,
      color:
        (matched?.track || seed.track) === "Partnership"
          ? MARKET_COLORS.amberBright
          : MARKET_COLORS.emeraldBright,
    };
...
function SignalRadar({
  ...
  signals: LiveOpportunitySignal[];
  summary: LeadSummary | null;
  loading: boolean;
  activeIndex: number;
}) {
  const radarRows = useMemo(
    () => buildRadarRows(signals, activeIndex),
    [signals, activeIndex]
  );
        ...
        <div className="mb-5 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-relaxed">
              ReadyForRobots helps robot companies find buyers for their robots.
              We monitor 150+ market signals to identify who is ready to
              purchase — and when the buying window opens.
            </p>
          </div>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
              ...
              <div key={String(label)} className="signal-radar-stat">
                <div className={`font-mono text-lg font-black ${colorClass}`}>
                  {value}
                </div>
      ...
      <div className="grid gap-4 p-5 lg:grid-cols-[1.35fr_0.85fr] lg:p-6">
        <div className="signal-radar-chart">
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, transparent, rgba(16,185,129,0.12), rgba(255,176,0,0.08), transparent)",
          ...
          <div className="relative mb-4 flex items-center justify-between text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">
            <span>
              {loading ? "Syncing scored leads" : "150+ sources scanning"}
            </span>
            <span className="text-emerald-400">Live signal strength</span>
          </div>
          <div className="signal-radar-chart-caption">
            Each bar is a signal type. Length shows buyer readiness from live
            sales leads — updated as we score the next opportunity.
          </div>

          <div className="relative space-y-4">
            {radarRows.map((row, rowIndex) => {
              const activeLane =
                activeSignal?.type === row.label &&
                activeSignal?.track === row.track;
              const sameMotion = activeSignal?.track === row.track;
              const adjustedValue = Math.min(
                0.98,
                Math.max(
                  0.16,
                  row.value + (activeLane ? 0.16 : sameMotion ? 0.05 : -0.03)
                )
              );
                        ...
                        >
                          <div
                            ...
                            style={{
                              ...
                              animation: activeLane
                                ? "rfr-bar-sheen 2.2s ease-in-out infinite"
                                : undefined,
                            }}
                          />
                        </div>
        ...
        <div className="signal-radar-live">
          {activeSignal && (
            <div className="relative">
              ...
              {activeSignal.robotTypes &&
                activeSignal.robotTypes.length > 0 && (
...
export default function Signals() {
  ...
  const [liveSignals, setLiveSignals] =
    useState<LiveOpportunitySignal[]>(fallbackLiveSignals);
  ...
  const publishSignalsTelemetry = () => {
    ...
    ).__rfrSignalsPollingTelemetry = {
      ...signalsTelemetryRef.current,
      summaryFailRate:
        summaryAttempts > 0
          ? Number((summaryFailures / summaryAttempts).toFixed(3))
          : 0,
      liveFailRate:
        liveAttempts > 0 ? Number((liveFailures / liveAttempts).toFixed(3)) : 0,
      breakerOpen: Date.now() < breakerOpenUntilRef.current,
      breakerOpenForMs: Math.max(0, breakerOpenUntilRef.current - Date.now()),
      summaryCooldownForMs: Math.max(
        0,
        summaryCooldownUntilRef.current - Date.now()
      ),
  ...
  const filtered = SIGNAL_TYPES.filter(s => {
    ...
    const matchInd =
      ...
      s.industries.includes("All industries");
    const matchSearch =
      search === "" ||
  ...
  useEffect(() => {
    ...
    async function loadLeadSummary() {
      ...
      const now = Date.now();
      if (
        now < breakerOpenUntilRef.current ||
        now < summaryCooldownUntilRef.current
      ) {
      ...
      const run = (async () => {
        try {
          ...
          if (!response.ok)
            throw new Error(`Lead summary failed with ${response.status}`);
          ...
          summaryCooldownUntilRef.current =
            Date.now() + SIGNALS_ENDPOINT_COOLDOWN_MS;
          maybeOpenBreaker();
          publishSignalsTelemetry();
        } finally {
    ...
    async function loadLiveSignals() {
      ...
      if (
        now < breakerOpenUntilRef.current ||
        now < liveCooldownUntilRef.current
      ) {
      ...
      const run = (async () => {
        try {
          ...
          if (!response.ok)
            throw new Error(`Live signal feed failed with ${response.status}`);
          const data = await response.json();
          const leads = Array.isArray(data) ? data : [];
          const mapped = leads
            .map(mapLeadToLiveSignal)
    ...
    const nextDelay = () => {
      if (Date.now() < breakerOpenUntilRef.current) {
        return Math.min(
          SIGNALS_MAX_POLL_MS,
          Math.max(
            SIGNALS_BASE_POLL_MS,
            breakerOpenUntilRef.current - Date.now()
          )
        );
      }
      const failStreak = Math.max(
        summaryFailStreakRef.current,
        liveFailStreakRef.current
      );
      if (failStreak <= 0) return SIGNALS_BASE_POLL_MS;
      return Math.min(
        SIGNALS_MAX_POLL_MS,
        SIGNALS_BASE_POLL_MS * 2 ** failStreak
      );
    };
      ...
      <PageHeroDark
        ...
        badge={
          <div className="page-hero-badge">
            ...
            {formatMetric(leadSummary?.total_signals ?? 0) || "8,058"} live
            signals · 150+ sources monitored
          </div>
        }
        ...
        stats={[
          {
            ...
            value: formatMetric(1204),
            tone: "white",
          },
          {
            label: "Hot leads",
            value: formatMetric(leadSummary?.hot ?? 38),
            tone: "amber",
          },
        ...
        <div className="container max-w-6xl pb-8">
          <div className="mb-8 flex flex-col gap-3 rounded-xl border border-emerald-200 bg-emerald-50/80 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-gray-800">
              These signals come from the same 150+ sources analyzed in our 2026
              Automation Imperative Report.
            </p>
            <Link
              href="/intelligence"
              className="inline-flex shrink-0 items-center gap-1.5 text-sm font-bold text-emerald-800 hover:text-emerald-900"
            >
          ...
          <SignalRadar
            signals={liveSignals}
            summary={leadSummary}
            loading={loadingLiveSignals}
            activeIndex={activeSignalIndex}
          />
            ...
            <div className="lg:col-span-2 flex flex-col gap-5">
              ...
              <div className="space-y-3">
                ...
                {filtered.map(sig => {
                  ...
                  return (
                    <div key={sig.id} className="surface-card overflow-hidden">
                      <button
