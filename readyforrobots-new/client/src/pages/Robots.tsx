...
function policyTone(country?: string): {
  label: string;
  color: string;
  bg: string;
} {
...
function StatusBadge({ status }: { status: string }) {
  const cfg: Record<string, { label: string; color: string; bg: string }> = {
    ...
    pilot: { label: "Pilot", color: "#fbbf24", bg: "rgba(251,191,36,0.15)" },
    research: {
      label: "Research",
      color: "#60a5fa",
...
function ConfidenceChip({ robot }: { robot: RobotRow }) {
  ...
  const color =
    label === "high" ? "#34d399" : label === "medium" ? "#fbbf24" : "#f87171";
  const bg =
    label === "high"
  ...
  const tip = [
    ...
    robot.heif_total_adjusted != null
      ? `Confidence-adjusted HEIF: ${robot.heif_total_adjusted}`
      : "",
  ]
    .filter(Boolean)
    .join("\n");
...
function RobotCard({ robot, rank }: { robot: RobotRow; rank: number }) {
  ...
  const recentTracked =
    isWithinDays(robot.created_at, 60) ||
    isWithinDays(robot.last_scraped_at, 60);
      ...
      >
        ...
        <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span
              className="rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider"
              style={{ color: policy.color, background: policy.bg }}
            >
              {policy.label}
            </span>
            {recentTracked ? (
            ...
            {specs?.total_dof != null ? (
              <span
                title={
                  specs.dof_note != null ? String(specs.dof_note) : undefined
                }
          ...
          <div className="mt-2 grid grid-cols-3 gap-x-3 gap-y-1 max-w-md">
            {HEIF_DIMS.map(d => (
              <div key={d}>
                <p className="text-[8px] text-gray-500 mb-0.5 uppercase tracking-wider">
                  {HEIF_LABELS[d]}
                </p>
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <span
            ...
            style={{
              color:
                robot.score_total >= 65
                  ? "#34d399"
                  : robot.score_total >= 45
                    ? "#fbbf24"
                    : "#f87171",
            }}
          >
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {(
                [
                  [
                    "Top speed",
                    specs.top_speed_mps != null
                      ? `${specs.top_speed_mps} m/s`
                      : null,
                    "top_speed_mps",
                  ],
                  [
                    "Payload",
                    specs.payload_kg != null ? `${specs.payload_kg} kg` : null,
                    "payload_kg",
                  ],
                  [
                    "Battery",
                    specs.battery_life_h != null
                      ? `${specs.battery_life_h} h`
                      : null,
                    "battery_life_h",
                  ],
                  [
                    "Charge time",
                    specs.charge_time_h != null
                      ? `${specs.charge_time_h} h`
                      : null,
                    "charge_time_h",
                  ],
                  [
                    "Height",
                    specs.height_cm != null ? `${specs.height_cm} cm` : null,
                    "height_cm",
                  ],
                  [
                    "Weight",
                    specs.weight_kg != null ? `${specs.weight_kg} kg` : null,
                    "weight_kg",
                  ],
                  [
                    "Fingers",
                    specs.finger_count != null
                      ? String(specs.finger_count)
                      : null,
                    "finger_count",
                  ],
                  [
                    "DOF",
                    specs.total_dof != null
                      ? `${specs.total_dof}${specs.dof_note != null ? " *" : ""}`
                      : null,
                    "total_dof",
                  ],
                  [
                    "Peak joint torque",
                    specs.peak_torque_nm != null
                      ? `${specs.peak_torque_nm} N·m`
                      : specs.peak_torque_note != null
                        ? String(specs.peak_torque_note)
                        : null,
                    "peak_torque_nm",
                  ],
                  [
                    "Price",
                    specs.price_usd != null
                      ? `$${Number(specs.price_usd).toLocaleString()}`
                      : "undisclosed",
                    "price_usd",
                  ],
                  [
                    "Stair climbing",
                    specs.can_climb_stairs != null
                      ? specs.can_climb_stairs
                        ? "Yes"
                        : "No"
                      : null,
                    "can_climb_stairs",
                ...
                .map(([label, value, key]) => {
                  ...
                  return (
                    <div
                      key={label}
                      className="rounded-lg px-2.5 py-1.5 bg-white/[0.03] border border-white/10"
                    >
                      ...
                      <p
                        ...
                        title={
                          key === "total_dof" && specs.dof_note != null
                            ? String(specs.dof_note)
                            : undefined
                        }
                      >
                        {String(value)}
...
export default function Robots() {
  const cachedRobots = readSurfaceCache<RobotRow[]>(
    ROBOTS_SURFACE_KEY,
    ROBOTS_SURFACE_TTL_MS
  );
  ...
  const filtered = robots
    ...
    .sort((a, b) => {
      ...
      if (sortDim === "heif_total") {
        return (
          (b.heif_total ?? b.score_total / 25) -
          (a.heif_total ?? a.score_total / 25)
        );
      }
      ...
      const bv =
        (b[heifKey] as number | undefined) ??
        ((b[scoreKey] as number | undefined) ?? 0) / 25;
      const av =
        (a[heifKey] as number | undefined) ??
        ((a[scoreKey] as number | undefined) ?? 0) / 25;
      return bv - av;
    });
    ...
    <div className="robots-page min-h-screen flex flex-col">
      ...
      <PageHeroDark
        ...
        badge={
          <div className="page-hero-badge">
            {robots.length || 109} humanoids benchmarked · HEIR 2026 · Updated
            monthly
          </div>
        }
        ...
        title={
          <>
            Humanoid Robot <span className="text-emerald-400">Index</span>
          </>
        }
        ...
        <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-bold text-gray-900">
              Ranked by HEIF · dual 0–4 and 0–100
            </h2>
            <p className="mt-1 text-[13px] text-gray-600">
              Known vendors use HEIR 2026 research scores; others infer from
              datasheets. Index = HEIF × 25.
            </p>
          </div>
      ...
      <section className="mx-auto max-w-5xl px-4 pb-16 text-sm text-gray-500">
        <p>
          ...
          <Link
            href="/find-robots"
            className="text-emerald-600/90 hover:text-emerald-700 underline underline-offset-4"
          >
